/**
 * AutoHealManager - compile error pattern matching and auto-fix engine.
 * Monitors build output, matches known error patterns, suggests/applies fixes.
 */
export interface HealPattern {
  name: string;
  regex: RegExp;
  suggestion: (match: RegExpMatchArray, filePath: string) => string;
  /** Optional auto-apply fix function returning new file content */
  autoFix?: (match: RegExpMatchArray, fileContent: string) => string | null;
}
...
export class AutoHealManager {
  ...
  register(pattern: HealPattern): void {
    this.patterns.push(pattern);
  }

  /** Analyze error output and generate fix suggestions */
  analyzeErrors(errors: string[], filePath?: string): HealSuggestion[] {
  ...
  getGenericHint(error: string): string {
  ...
  categorizeErrors(errors: string[]): { syntax: number; type: number; import: number; other: number } {
    const result = { syntax: 0, type: 0, import: 0, other: 0 };
    for (const err of errors) {
      if (/syntax|unexpected|parse/i.test(err)) result.syntax++;
      else if (/type|cannot|not assignable/i.test(err)) result.type++;
      else if (/import|cannot find module|not found/i.test(err)) result.import++;
      else result.other++;
    }
    return result;
  }
