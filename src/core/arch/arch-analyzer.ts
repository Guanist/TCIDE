...
export interface DepNode {
  ...
  imports: string[];
  exports: string[];
  depCount: number; // 被依赖次数
}

export interface SmellItem {
  file: string;
  type: 'long-file' | 'long-function' | 'too-many-params' | 'deep-nesting' | 'todo-fixme' | 'circular-dep' | 'large-class' | 'magic-number';
  line: number;
  severity: 'warn' | 'error';
  message: string;
}
...
const IMPORT_PATTERNS: Array<{ exts: string[]; regex: RegExp; group: number }> = [
  // Python
...
const SMELL_PATTERNS = [
  { type: 'todo-fixme' as const, regex: /\b(TODO|FIXME|HACK|XXX)\b/gi, severity: 'warn' as const, msg: '未完成的 TODO/FIXME' },
  { type: 'magic-number' as const, regex: /\b(?!0\b|1\b|2\b|10\b|100\b|60\b|24\b)\d{3,}\b/g, severity: 'warn' as const, msg: '魔法数字' },
];

export class ArchitectureAnalyzer {
  private nodes = new Map<string, DepNode>();
  private edges: DepEdge[] = [];
  private smells: SmellItem[] = [];
  ...
  async analyze(): Promise<ArchOverview> {
  ...
  private async walkAndAnalyze(dir: string, depth = 0): Promise<void> {
    if (depth > 10) return;
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        try {
          ...
          if (stat.size > 500 * 1024) continue; // skip > 500KB
  ...
  private extractImports(content: string, ext: string): string[] {
    ...
    for (const pattern of IMPORT_PATTERNS) {
      if (!pattern.exts.includes(ext)) continue;
  ...
  private extractExports(content: string, lang: string): string[] {
    ...
    if (['kotlin', 'java'].includes(lang)) {
      patterns.push(
        { regex: /^(?:(?:data\s+|sealed\s+|abstract\s+|open\s+)?class|interface|object|enum\s+class)\s+(\w+)/gm, nameGroup: 1 },
        { regex: /^(?:suspend\s+)?fun\s+(\w+)/gm, nameGroup: 1 },
      );
    } else if (['typescript', 'javascript'].includes(lang)) {
      patterns.push(
        { regex: /^export\s+(?:default\s+)?(?:class|interface|function|const|let|var|type|enum)\s+(\w+)/gm, nameGroup: 1 },
      );
    } else if (lang === 'python') {
      patterns.push(
        { regex: /^(?:class|def)\s+(\w+)/gm, nameGroup: 1 },
      );
    }
  ...
  private detectSmells(relPath: string, content: string, lines: string[]): void {
    // 大文件 (>500 行)
    if (lines.length > 500) {
    ...
    for (const pattern of SMELL_PATTERNS) {
      for (const m of content.matchAll(pattern.regex)) {
        ...
        this.smells.push({ file: relPath, type: pattern.type as SmellItem['type'], line, severity: 'warn',
    ...
    for (const line of lines) {
      ...
      if (indent > 4) indentCounts.push(indent);
    }
  ...
  private resolveDeps(): void {
    ...
    const pathIndex = new Map<string, string>(); // 短路径 → 完整路径
    ...
    for (const node of nodeList) {
      if (node.type === 'file') {
        ...
        if (noExt !== node.name) pathIndex.set(noExt, node.id);
      }
    }

    const resolved: DepEdge[] = [];
  ...
  private findEntryPoints(): string[] {
    ...
    for (const c of candidates) {
      ...
      if (fs.existsSync(full)) {
        ...
        if (fs.statSync(full).isDirectory()) {
          ...
          for (const f of files.slice(0, 3)) entries.push(c + '/' + f);
        } else {
          entries.push(c);
        }
      }
    ...
    if (entries.length === 0) {
      ...
      for (const n of nodeList.slice(0, 3)) entries.push(n.path);
    }
    return entries;
  }

  private dirHasSourceFiles(dir: string): boolean {
    const exts = ['.kt', '.java', '.ts', '.tsx', '.js', '.py', '.go', '.rs'];
    try {
      for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
        if (entry.isFile() && exts.includes(path.extname(entry.name).toLowerCase())) return true;
      }
    } catch { return false; }
    return false;
  }
  ...
  private detectProjectType(): string {
    if (fs.existsSync(path.join(this.projectRoot, 'build.gradle.kts')) ||
      ...
      if (fs.existsSync(path.join(this.projectRoot, 'app', 'src', 'main', 'AndroidManifest.xml'))) return 'android';
      return 'gradle';
    }
    ...
    if (fs.existsSync(path.join(this.projectRoot, 'pyproject.toml')) ||
        fs.existsSync(path.join(this.projectRoot, 'setup.py'))) return 'python';
    return 'generic';
  }
}
