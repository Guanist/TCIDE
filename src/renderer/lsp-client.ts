...
function toLspUri(uri: monaco.Uri): string {
  return uri.toString();
}
...
function fromLspPosition(pos: { line: number; character: number }): monaco.Position {
  return new monaco.Position(pos.line + 1, pos.character + 1);
}

/** 将 LSP Range 转为 Monaco Range */
function fromLspRange(range: { start: { line: number; character: number }; end: { line: number; character: number } }): monaco.Range {
...
// ── 诊断存储 (跨模型) ──
const diagnosticsMap = new Map<string, monaco.editor.IMarkerData[]>();
...
class LspClient {
  ...
  async start(projectPath: string): Promise<void> {
    ...
    if (!available) {
      ...
      if (guide && (window as any).showToast) {
        (window as any).showToast(`📦 ${lspLang} 服务未安装 → ${guide}`, 'warning', 8000);
      }
      return;
    }
  ...
  /** 通知服务器文档已打开 */
  async didOpen(uri: monaco.Uri, language: string, text: string): Promise<void> {
  ...
  /** 通知服务器文档已修改 */
  async didChange(uri: monaco.Uri, contentChanges: Array<{ text: string }>): Promise<void> {
    if (!this.running) return;
  ...
  /** 通知服务器文档已关闭 */
  async didClose(uri: monaco.Uri): Promise<void> {
    if (!this.running) return;
    const lspLang = LANG_MAP[this.getLanguage()];
    if (!lspLang) return;
  ...
  /** 处理来自服务器的消息 */
  private handleServerMessage(data: { language: string; message: any }): void {
  ...
  /** 应用诊断标记到 Monaco */
  private applyDiagnostics(uri: string, lspDiagnostics: any[]): void {
    const markers: monaco.editor.IMarkerData[] = lspDiagnostics.map((d: any) => ({
    ...
    const completionProvider = monaco.languages.registerCompletionItemProvider(langId, {
      ...
      provideCompletionItems: async (model, position, context) => {
        try {
          ...
          if (!resp.success || !resp.result) return { suggestions: [] };

          const result = resp.result as any;
          const items: monaco.languages.CompletionItem[] = (result.items || []).map((item: any) => ({
            label: item.label,
    ...
    const refProvider = monaco.languages.registerReferenceProvider(langId, {
      provideReferences: async (model, position, _context) => {
        try {
          ...
          const result = resp.result as Array<any>;
    ...
    const symProvider = monaco.languages.registerDocumentSymbolProvider(langId, {
      provideDocumentSymbols: async (model) => {
        try {
          ...
          const result = resp.result as Array<any>;
          return result.map((sym: any) => this.convertSymbol(sym));
        } catch (e) {
          return [];
        }
      },
    ...
    const sigProvider = monaco.languages.registerSignatureHelpProvider(langId, {
      ...
      provideSignatureHelp: async (model, position, _token, _context) => {
        try {
          ...
          if (!resp.success || !resp.result) return { value: { signatures: [], activeSignature: 0, activeParameter: 0 }, dispose: () => {} };
  ...
  private lspCompletionKind(kind: number | undefined): monaco.languages.CompletionItemKind {
  ...
  private convertSymbol(sym: any): monaco.languages.DocumentSymbol {
    return {
      name: sym.name,
      detail: sym.detail || '',
      ...
      children: (sym.children || []).map((c: any) => this.convertSymbol(c)),
    };
  }

  private lspSymbolKind(kind: number): monaco.languages.SymbolKind {
...
/** 获取或创建 LSP 客户端 */
export function getLspClient(language: string): LspClient {
...
/** 文档打开时调用 */
export async function lspDidOpen(uri: monaco.Uri, language: string, text: string): Promise<void> {
  const client = getLspClient(language);
  await client.didOpen(uri, language, text);
}

/** 文档修改时调用 */
export async function lspDidChange(uri: monaco.Uri, language: string, changes: Array<{ text: string }>): Promise<void> {
  const client = getLspClient(language);
  await client.didChange(uri, changes);
}

/** 文档关闭时调用 */
export async function lspDidClose(uri: monaco.Uri, language: string): Promise<void> {
  const client = getLspClient(language);
  await client.didClose(uri);
}

/** 获取某语言的诊断信息 */
export function getDiagnostics(uri: string): monaco.editor.IMarkerData[] {
  return diagnosticsMap.get(uri) || [];
}

/** 获取所有语言的诊断信息 */
export function getAllDiagnostics(): monaco.editor.IMarkerData[] {
