...
export type LspLanguage = 'python' | 'go' | 'rust' | 'cpp' | 'java' | 'bash';
...
const LSP_CONFIGS: Record<LspLanguage, LspServerConfig> = {
  python: {
    language: 'python',
    command: 'npx',
...
function filePathToURI(fp: string): string {
...
class LspManager {
  ...
  onServerMessage: ((language: string, message: unknown) => void) | null = null;
  ...
  isAvailable(language: LspLanguage): boolean {
    ...
    for (const searchPath of searchPaths) {
      try {
        ...
        if (result.toString().trim().length > 0) return true;
      } catch { /* continue */ }
    }
    return false;
  }
  ...
  /** 获取某语言服务器安装指引 */
  getInstallGuide(language: LspLanguage): string {
  ...
  /** 获取常见安装搜索路径 */
  private getSearchPaths(language: LspLanguage, cmd: string): string[] {
    const paths = [cmd];
  ...
  /** 启动语言服务器 */
  async startServer(language: LspLanguage, projectPath: string): Promise<void> {
    // 如果已有同语言同项目的服务器，先停止
    ...
    proc.on('exit', (code, signal) => {
      ...
      // 拒绝所有待处理的请求
      state.pendingRequests.forEach(({ reject }) =>
    ...
    proc.stdout?.on('data', (chunk: Buffer) => {
      ...
      while (true) {
        ...
        if (state.buffer.length < bodyEnd) break; // 等待更多数据
    ...
    console.log(`[LSP] ${language} 服务器就绪 (${(initResult as any).serverInfo?.name || 'unknown'})`);
  ...
  /** 停止语言服务器 */
  async stopServer(language: string, projectPath?: string): Promise<void> {
  ...
  /** 获取服务器状态 */
  getStatus(language: string, projectPath?: string): { running: boolean; serverName?: string } {
    const key = projectPath ? `${language}:${projectPath}` : language;
    if (!projectPath) {
      for (const [k] of this.servers) {
        if (k.startsWith(language + ':')) return { running: true };
      }
  ...
  /** 向服务器发送 LSP 请求并等待响应 */
  sendLspRequest(language: string, method: string, params: unknown, projectPath?: string): Promise<unknown> {
    const state = this.findServer(language, projectPath);
    if (!state) throw new Error(`LSP server not running for ${language}`);
    return this.sendRequest(state, method, params);
  }

  /** 向服务器发送 LSP 通知 (无响应) */
  sendLspNotification(language: string, method: string, params: unknown, projectPath?: string): void {
    const state = this.findServer(language, projectPath);
    if (!state) {
  ...
  private findServer(language: string, projectPath?: string): LspServerState | undefined {
    if (projectPath) return this.servers.get(`${language}:${projectPath}`);
    // 查找该语言的任何实例
    for (const [k, v] of this.servers) {
      if (k.startsWith(language + ':')) return v;
    }
    return undefined;
  }

  private killServer(key: string, state: LspServerState): void {
    try {
      state.process.kill();
    } catch {
      // 进程可能已经退出
  ...
  private handleMessage(state: LspServerState, key: string, msg: any): void {
  ...
  private sendRequest(state: LspServerState, method: string, params: unknown): Promise<unknown> {
  ...
  private sendNotification(state: LspServerState, method: string, params: unknown): void {
  ...
  private getInitializationOptions(language: LspLanguage, projectPath: string): any {
    switch (language) {
      case 'python':
        return {
