...
class UnattendedRunner {
    ...
    async execute(plan, options = {}) {
        if (this.isRunning)
            throw new Error('Runner already executing');
        this.isRunning = true;
        ...
        for (let i = 0; i < plan.length; i++) {
            ...
            while (retries <= MAX_RETRIES && !success) {
                ...
                try {
                    ...
                    const result = await this._withTimeout(step.action(this.projectRoot, { step, runner: this }), STEP_TIMEOUT);
                    // Validate
    ...
    async _validate(step) {
        ...
        if (step.validate?.lint) {
            try {
                ...
                try {
                    ...
                    if (errCount > 0)
                        errors.push(`${errCount} lint errors`);
                    const warnCount = parsed.reduce((sum, f) => sum + (f.warningCount || 0), 0);
                    if (warnCount > 0)
                        warnings.push(`${warnCount} lint warnings`);
                }
                catch {
        ...
        if (step.validate?.tsc && fs.existsSync(path.join(this.projectRoot, 'tsconfig.json'))) {
            try {
                ...
                if (errLines > 0)
                    errors.push(`${errLines} TypeScript errors`);
            }
        }
        // 3. Build
