...
class ProjectMemory {
    ...
    recordPattern(trigger, solution, context, source = 'conversation') {
        ...
        // Check duplicate
        const exists = this.memory.patterns.find(p => p.trigger === trigger || (solution && p.solution?.slice(0, 100) === solution?.slice(0, 100)));
        if (exists) {
    ...
    searchPatterns(query) {
        ...
        return this.memory.patterns
            .filter(p => p.trigger?.toLowerCase().includes(queryLower) ||
    ...
    reset() {
        ...
        if (this.memoryDir && fs.existsSync(this.memoryDir)) {
            try {
                fs.rmSync(this.memoryDir, { recursive: true });
            }
            catch { }
        }
    }
        ...
        if (fs.existsSync(pkgJson)) {
            try {
                ...
                if (pkg.dependencies) {
                    ...
                    if (pkg.dependencies.tauri || pkg.dependencies['@tauri-apps/cli'])
                        stack.frameworks.push('Tauri');
                    if (pkg.dependencies.three)
                        stack.frameworks.push('Three.js');
                    if (pkg.dependencies.express || pkg.dependencies.koa) {
                        if (pkg.dependencies.prisma || pkg.dependencies.typeorm || pkg.dependencies.knex)
                            stack.databases.push('SQL (ORM)');
                    }
                    if (pkg.dependencies.mongoose || pkg.dependencies.mongodb)
                        stack.databases.push('MongoDB');
                    if (pkg.dependencies.redis || pkg.dependencies.ioredis)
                        stack.databases.push('Redis');
                    if (pkg.dependencies.pg || pkg.dependencies['node-postgres'])
                        stack.databases.push('PostgreSQL');
                    if (pkg.dependencies.mysql2 || pkg.dependencies.mysql)
                        stack.databases.push('MySQL');
                    // Build tools
                    if (pkg.dependencies.webpack || pkg.devDependencies?.webpack)
                        stack.buildTools.push('Webpack');
                    if (pkg.dependencies.vite || pkg.devDependencies?.vite)
                        stack.buildTools.push('Vite');
                    if (pkg.dependencies.turbo || pkg.devDependencies?.turbo)
                        stack.buildTools.push('Turbo');
    ...
    _detectCodingStyle() {
        ...
        if (fs.existsSync(editorConfig)) {
            try {
                ...
                if (indentMatch)
                    style.indent = `${indentMatch[1]} spaces`;
                ...
                try {
                    ...
                    if (f.endsWith('.json')) {
                        ...
                        if (content.includes('singleQuote'))
                            style.quotes = content.includes("singleQuote: true") ? 'single' : 'double';
                        if (content.includes('semi'))
                            style.semicolons = content.match(/['"]semi['"]\s*:\s*false/) ? false : true;
                        if (content.includes('max-len')) {
                            const m = content.match(/max-len['"]\s*:\s*\[.*?(\d+)/);
                            if (m)
                                style.maxLineLength = parseInt(m[1]);
                        }
                    }
                }
                catch { }
            ...
            if (fs.existsSync(fp)) {
                try {
                    ...
                    if (config.semi !== undefined)
                        style.semicolons = config.semi;
                    if (config.tabWidth)
                        style.indent = `${config.tabWidth} spaces`;
                    if (config.printWidth)
                        style.maxLineLength = config.printWidth;
                }
                catch { }
                break;
            }
    ...
    _detectPackageManager() {
        ...
        if (fs.existsSync(path.join(this.projectRoot, 'bun.lockb')))
            return 'bun';
        if (fs.existsSync(path.join(this.projectRoot, 'Cargo.lock')))
            return 'cargo';
        if (fs.existsSync(path.join(this.projectRoot, 'go.sum')))
            return 'go mod';
        return 'unknown';
    }
    _formatInjection() {
        ...
        if (m.techStack) {
            injection += `\n## 技术栈\n`;
            if (m.techStack.languages?.length)
                injection += `- 语言: ${m.techStack.languages.join(', ')}\n`;
            if (m.techStack.frameworks?.length)
                injection += `- 框架: ${m.techStack.frameworks.join(', ')}\n`;
            if (m.techStack.buildTools?.length)
                injection += `- 构建工具: ${m.techStack.buildTools.join(', ')}\n`;
            if (m.techStack.databases?.length)
                injection += `- 数据库: ${m.techStack.databases.join(', ')}\n`;
        }
        if (m.packageManager)
        ...
        if (m.directoryLayout) {
            ...
            if (m.directoryLayout.hasSrc)
                injection += `- 源码在 src/ 下\n`;
            if (m.directoryLayout.hasTests)
                injection += `- 有测试目录\n`;
        }
    ...
    _saveMemory() {
        if (!this.memoryDir)
            return;
        try {
            fs.writeFileSync(path.join(this.memoryDir, 'memory.json'), JSON.stringify(this.memory, null, 2), 'utf-8');
        }
        catch (err) {
