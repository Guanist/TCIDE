...
class GitIntelligence {
    ...
    async generateCommitMessage(projectRoot, options = {}) {
        ...
        let rawDiff = '';
        try {
            rawDiff = this._execArgs(['diff', '--cached', '-U3'], projectRoot) || '';
        }
        catch { }
        if (!rawDiff) {
            try {
                rawDiff = this._execArgs(['diff', '-U3'], projectRoot) || '';
            }
            catch { }
        }
    ...
    _conventionalBreakdown(files, diff) {
        ...
        for (const file of files) {
            ...
            if (!fileTypes.has(type))
                fileTypes.set(type, { type, scope, files: [], totalChanges: 0 });
    ...
    _generateSummary(files, stats) {
        ...
        if (stats.additions)
            parts.push(`${stats.additions} additions`);
        if (stats.deletions)
            parts.push(`${stats.deletions} deletions`);
        if (stats.binary)
            parts.push(`${stats.binary} binary files`);
        const byType = {};
