...
class SmartTrimmer {
    ...
    setProjectEntropy(entropy) {
        ...
        if (entropy > 70)
            this.config.trimIntervalMs = 120000;
        else if (entropy > 50)
            this.config.trimIntervalMs = 180000;
        else
            this.config.trimIntervalMs = 300000;
    }
    /**
     * 获取归档摘要
     */
    ...
    _calculateBudget(context) {
        ...
        if (this.projectEntropy > 70)
            budget *= 0.7;
        else if (this.projectEntropy > 45)
            budget *= 0.85;
    ...
    _minimal(chunk) {
        ...
        const parts = [];
        if (fileMatch)
            parts.push(`files: ${fileMatch.slice(0, 3).join(', ')}`);
        if (funcMatch)
            parts.push(funcMatch.slice(0, 3).join(', '));
    ...
    _archive(removed) {
        ...
        if (this.archiveIndex.length > 1000)
            this.archiveIndex = this.archiveIndex.slice(-1000);
        this._saveArchive();
    }
    _saveArchive() {
        if (!this.projectRoot)
            return;
        try {
            fs.writeFileSync(path.join(this.projectRoot, '.tcide', 'trimmer', 'smart_archive.json'), JSON.stringify(this.archiveIndex.slice(-200), null, 0), 'utf-8');
        }
        catch { }
    }
    _loadArchive() {
        ...
        try {
            ...
            if (fs.existsSync(fp))
                this.archiveIndex = JSON.parse(fs.readFileSync(fp, 'utf-8'));
        }
        catch { }
    }
}
