...
class EntropyEvaluator {
    ...
    async evaluate(onProgress) {
        ...
        const totalEntropy = Math.round(dimensions.complexity.score * weights.complexity +
    ...
    _analyzeComplexity(content, file) {
        ...
        for (const line of lines) {
            ...
            if (/\bswitch\b/.test(trimmed))
                complexity++;
            if (/\bcase\b/.test(trimmed) && !trimmed.startsWith('switch'))
                complexity++;
            ...
            // Function count
            if (/\b(function|def|func|fun|=>|class|interface|struct)\b/.test(trimmed))
                functionCount++;
        }
        ...
        for (const file of files.slice(0, 300)) {
            try {
                ...
                for (const imp of imports) {
                    if (!importsByFile.has(imp))
                        importsByFile.set(imp, []);
    ...
    _analyzeChurn() {
        ...
        try {
            ...
            const output = child_process.execSync('git log --pretty=format: --name-only --since="90 days ago" | sort | uniq -c | sort -rn | head -30', { cwd: this.projectRoot, encoding: 'utf-8', timeout: 10000 });
            ...
            try {
                for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
                    ...
                    if (entry.isDirectory())
                        walk(full, depth + 1);
                    else if (INDEX_EXTS.has(path.extname(entry.name).toLowerCase()))
                        files.push(full);
                }
            }
            catch { }
