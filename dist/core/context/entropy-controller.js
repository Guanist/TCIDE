...
class EntropyController {
    ...
    getSystemPromptInjection() {
        if (this.contextEntropy < ENTROPY_THRESHOLDS.warn)
            return '';
    ...
    _computeContextEntropy(messages, projectEntropy) {
        ...
        const entropy = densityFactor * weights.density +
    ...
    _computeAdaptiveBudget(projectEntropy, taskComplexity) {
        ...
        if (projectEntropy > 70)
            budget *= 0.6;
        else if (projectEntropy > 45)
            budget *= 0.8;
    ...
    _decayRelevance(messages) {
        ...
        for (let i = 0; i < messages.length; i++) {
            ...
            if (i >= messages.length - 3)
                boost = 3.0; // last 3 messages
            if (i >= messages.length - 8)
                boost = 1.5; // last 8 messages
    ...
    _allocateAttentionBudget(messages, currentTask, openFiles) {
        ...
        for (const topic of topics) {
            let boost = 1.0;
            if (currentTask && topic.name.toLowerCase().includes(currentTask.toLowerCase()))
                boost = 3.0;
            if (openFiles.some(f => topic.name.includes(path.basename(f))))
                boost = 2.5;
    ...
    _extractTopics(messages, currentTask, openFiles) {
        ...
        for (const msg of messages) {
            ...
            for (const word of words) {
                if (['the', 'and', 'for', 'that', 'this', 'with', 'from', 'have', 'are'].includes(word))
                    continue;
                if (!topics.has(word))
                    topics.set(word, { name: word, frequency: 0, lastSeen: 0 });
