...
export class BuilderAgent {
  ...
  async run(requirement: string, projectContext: object): Promise<Task[]> {
    // 构造四层上下文 Prompt
  ...
  private buildContextPrompt(context: object): string {
    const parts: string[] = [];
  ...
  private parseTaskList(response: string): Task[] {
    // 尝试多种方式提取 JSON
